...
import { STABLECOIN_RATINGS, getGradeColor } from '@/data/stablecoin-ratings';
    ...
    <div className="min-h-screen bg-[#030308] text-[#E0E0FF] relative">
      ...
      <div className="relative z-10 max-w-3xl mx-auto px-4 py-6 md:px-8 md:py-10">
        ...
        <div className="flex items-center gap-2 md:gap-4 mb-6 flex-wrap">
          <Link
            href="/?view=premium"
            className="inline-flex items-center gap-1.5 text-[10px] md:text-[11px] font-mono tracking-wider text-[#7070AA] hover:text-[#00F5FF] transition-colors"
          >
        ...
        {/* Popular stablecoins with Pulse ratings */}
        {regulation?.stablecoinsAllowed && regulation.stablecoinsAllowed.length > 0 && (() => {
          const matched = regulation.stablecoinsAllowed
            .map((ticker) => STABLECOIN_RATINGS.find((r) => r.ticker === ticker))
            .filter(Boolean);
            ...
            >
              ...
              <div className="space-y-2">
                {matched.map((rating) => {
                  ...
                  const color = getGradeColor(rating.grade);
                  return (
                    <Link
                      key={rating.id}
                      href="/ratings"
                    ...
                    >
                      ...
                      >
                        <span className="text-sm font-mono font-bold" style={{ color }}>{rating.grade}</span>
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] text-[#7070AA]">{rating.name}</span>
                        </div>
                        <div className="text-[9px] font-mono text-[#7070AA]">
                          {rating.issuer} · Pulse Score {rating.totalScore}/100
                        </div>
                      </div>
                      <svg
                        width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"
                        className="text-[#7070AA] group-hover:text-[#00F5FF] transition-colors flex-shrink-0"
                      >
              ...
              <Link
                href="/ratings"
                className="inline-block text-[9px] font-mono text-[#7070AA] hover:text-[#00F5FF] transition-colors mt-2"
              >
                See all {STABLECOIN_RATINGS.length} rated stablecoins →
              </Link>
            </section>
          );
        })()}
